...
# GPUS="0 1 2 3" (env) selects the GPUs; decodes are sharded across them, scoring uses one GPU per condition (round-robin) + one for the VLM pass.
set -u
CTRL=$1; CONDS=$2
...
dec() { CUDA_VISIBLE_DEVICES=$1 python decode_pid.py --controller $CTRL --condition $2 --ks 28,24,16 --nshards $NG --shard $3 2>&1 | grep -v -i -E "warning|pynvml|INFO|Time spent" > $LOG/${CTRL}_block_decode_$2_$3.log; }
for c in $CONDS; do                       # one condition at a time, sharded across all selected GPUs
  for ((k=0; k<NG; k++)); do dec ${G[$k]} $c $k & done
  wait
done
...
for c in $CONDS; do
  ...
  ( std ${G[$((i % NG))]} $c $s ) 2>&1 | grep -v -i -E "warning|pynvml|Loading checkpoint" > $LOG/${CTRL}_block_score_$c.log &
  i=$((i+1))
done
if echo "$CONDS" | grep -q canny; then ( vlm ${G[$((i % NG))]} ) 2>&1 | grep -v -i -E "warning|pynvml|Loading checkpoint" > $LOG/${CTRL}_block_score_vlm.log & fi
wait
