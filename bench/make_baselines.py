...
F2 DIV8K real high-resolution validation (when present), G efficiency, H the rule for "beating" a baseline. Dagger = bicubic x4 upsample of a 512-native output (interpolation route, not native).
"""
import csv
import glob
import json
...
row("PiD round trip (generative ceiling; 0.80 = practical ceiling of the 2048 / c512 column; its c2048 value is the sampler-noise floor, not 1.0, since v1.16)", get(M, "canny", "", "pid_roundtrip", 512), get(M, "canny", "", "pid_roundtrip", 2048), get(M, "depth", "", "pid_roundtrip", 512), get(ADE, "seg", "", "pid_roundtrip", 512), nat=get(M, "canny", "", "pid_roundtrip", 2048, cond_res=2048))
...
if has_t:
    ...
    L += ["", "Reading: the teacher sits at or slightly below the student at the matched view under every controller and falls further behind at native resolution, most against c2048, "
          "whose reference is a STUDENT round trip at the held-out decoder seed 7 (decoder-family-specific map; v1.16). Teacher decode 12.7 s vs 1.0 s.", ""]

# ---------------------------------------------------------------- F2. DIV8K real high-resolution validation (v1.13; when present)
LS = "div8k1k"
if any(k[0] == LS for k in recs):
    L += ["## F2. Validation of the c2048 column on REAL 2048 photographs (DIV8K split div8k1k, OminiControl canny; the real image is the reference at 1.0)", "",
    ...
    L += ["", "Reading: on multigen5k the c2048 map is synthesized by a PiD round trip at the held-out decoder seed 7, so no evaluated row scores 1.0 there (the seed-0 round trip scores about 0.71, the column's sampler-noise floor); here c2048 is the edge map of the "
