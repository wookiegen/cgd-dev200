...
| Bbox layout (DROPPED from the paper 2026-09-12; manifests and scorer kept, unused) | `coco_val5k` = COCO 2017 val, 5000 (4925 with boxes after the crop) | `train/coco_train` = COCO 2017 train, 118,287 rows | GT boxes rendered with `coco_val5k/class_colors.json` | `IDEA-Research/grounding-dino-base`, success rate @ IoU 0.5 and mIoU |
...
| COCO 2017 (no longer used by the paper since 2026-09-12; kept for now, TO BE DELETED after the submission) | images.cocodataset.org (`val2017.zip`, `train2017.zip`, `annotations_trainval2017.zip`) | 2017 release | `/data/wookiekim/cgd/data/coco2017/{val2017,train2017,annotations,zips}` | 39 GB | 5000 val + 118,287 train images, instances + captions for both |
