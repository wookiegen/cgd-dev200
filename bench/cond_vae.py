...
class CondVAEDecoder(nn.Module):
    ...
    def __init__(self, decoder: nn.Module, post_quant_conv: nn.Module | None = None, in_ch: int = 3):
        super().__init__()
        self.dec = decoder; self.post_quant_conv = post_quant_conv
        ...
        self.inject = nn.ModuleList([_zero(nn.Conv2d({64: w[3], 128: w[2], 256: w[1], 512: w[0]}[r], c, 1)) for r, c in zip(res_after, ch_after)])
    ...
    def forward(self, z: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        x = x + self.inject[0](feats[self.res_after[0]])
        for i, up in enumerate(d.up_blocks):
            x = up(x, None)
            if i < 3:
                x = x + self.inject[i + 1](feats[self.res_after[i + 1]])
        x = d.conv_norm_out(x); x = d.conv_act(x); x = d.conv_out(x)
        return x

    def trainable_parameters(self, decoder: bool = True):
        ps = list(self.branch.parameters()) + list(self.inject.parameters())
        if decoder:
...
def load_cond_vae(vae, ckpt_path: str | None = None, device="cuda", dtype=torch.float32) -> CondVAEDecoder:
    """Build the conditioned decoder around a diffusers AutoencoderKL (FLUX); optionally load a trained state dict (safetensors)."""
    m = CondVAEDecoder(vae.decoder, getattr(vae, "post_quant_conv", None))
    if ckpt_path:
        from safetensors.torch import load_file
        missing, unexpected = m.load_state_dict(load_file(ckpt_path), strict=False)
