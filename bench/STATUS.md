...
Running at hand-off (2026-09-12 13:20 KST, detached inside the container): (a) **condition-scale sweep** for fig:ceiling on GPUs 0,1 (`run/run_sweep_condition_scale.sh`, scales 0 / 0.25 / 0.5 / 0.75 / 1 / 1.5 / 2 / 4 on the canny dev-200, VAE + PiD K = 28 / 24; ~8.4 s per image, ETA ~15:10 KST; marker `done_sweep_condition_scale.txt`, results `results/bench/sweep/`); (b) **DreamBench subject track** (user order 2026-09-12: sweep, then subject rows, then the conditioned VAE decoder; the CGD training targets are POSTPONED): a 50-image prompt A/B (class-name vs "this item") on GPU 3, then `run/run_subject_block.sh omini` on GPUs 2,3 (marker `done_subject_omini.txt`). All three controller blocks finished earlier (`done_omini_block.txt`, `done_fluxcn_block.txt` 02:22 KST, `done_easycontrol_block.txt` 05:42 KST) and are filled in the paper. For future blocks (seg / bbox LoRAs): `run/run_block.sh <ctrl> "<conds>"` with `GPUS`; every stage skips existing outputs, so re-running the same command resumes it. A block takes ~13 h on two GPUs (three scoring chains) and ~8 h on four.
